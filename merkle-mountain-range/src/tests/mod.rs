...
mod bytes_mt_tests {
    ...
    struct JubJubMerkleMountrainRangeParams;

    impl Config for JubJubMerkleMountrainRangeParams {
        type Leaf = [u8];
    ...
    type JubJubMerkleMountrainRange = MerkleMountrainRange<JubJubMerkleMountrainRangeParams>;
    ...
    fn merkle_tree_test<L: CanonicalSerialize>(leaves: &[L], update_query: &[(usize, L)]) -> () {
        ...
        let two_to_one_params = <CompressH as TwoToOneCRHScheme>::setup(&mut rng)
            .unwrap()
            .clone();
        let mut tree = JubJubMerkleMountrainRange::new(
...
mod field_mt_tests {
    ...
    type FieldMT = MerkleMountrainRange<FieldMTConfig>;
